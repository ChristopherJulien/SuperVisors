...
//Analog pins
#define phototransistorA_pin    A2 // Front phototransistor
#define phototransistorB_pin    A1 // Left phototransistor
#define phototransistorC_pin    A0 // Back phototransistor
#define phototransistorD_pin    A3 // Right phototransistor
#define SDA_pin                 A4 
#define SCL_pin                 A5
#define rain_pin                A6
...
long previousTimeLuminosity = -10;
...
long previousTimePotentiometer = 0;
...
static float opaque_max = 90;              //angle with the maximum tint
int gradient;                           //gradient will vary between opaque_min and opaque_max
static float angle;

//rain variables
int rain_threshold = 900;
...
static int luminosity_threshold;
...
//
      flag_sun = checkSun(moyenne_phototransistors, luminosity_threshold);  //test if the luminosity is higher than the threshold
...
//      Serial.println(pressure);
      ...
      flag_vel = checkVelocity(pressure, velocity_threshold);                      //test if the velocity is higher than the threshold
...
//  //Measure RAIN every 500ms
  if(currentTime- previousTimeRain >= rainInterval) {
    flag_rain = checkRain();                              //test if there is rain
...
//    Serial.println(pressure);
      ...
      pot0 = ads1015.readADC_SingleEnded(0); //potentiometer for luminosity
...
//      if(luminosity_pot_value > 4000) luminosity_pot_value = 0;
      luminosity_threshold = map(pot0, 0, 1100, 10, 900); //maps the value of the potentiometer to a value between 300 and 900
      velocity_threshold = map(pot1,0,1103,5,25);
  ...
  if(currentTime - previousTimeBraking >= brakingInterval) {
        if ( IMU.accelerationAvailable()) {
          ...
          if( abs(acc_x) > braking_threshold){
            doSomethingWithTheBrakingLED(1);
        }
        else{
          doSomethingWithTheBrakingLED(0);
          }

        previousTimeBraking =currentTime;  
  }
  

  //recompute state every 100 ms and move motor accordingly
  if(stateFlag == 1) {
    state = checkState(flag_vel,flag_rain,flag_sun);

//    Serial.print("flag_vel : ");
//    Serial.print(flag_vel);
//    Serial.print("flag_rain : ");
//    Serial.print(flag_rain);
//    Serial.print(" flag_sun : ");
//    Serial.println(flag_sun);
    ...
    switch (state){
      case OPEN:
        if (angle != angleOpen){                         //test if the visor is open and open it if necessary
          Serial.println("Hellooooooooooooooooooooooooooooooooooooooooooooo");
           servoRight.write(0 + angleOpen); // par ex 90 + desired_angle
           servoLeft.write(180 - angleOpen);
           angle = angleOpen;
        }
        break;
      case TRANSPARENT:
        if (angle != angleTransparent){                  //test if the visor is in transparent state and move it if necessary
          Serial.println("lolololololololololololololololololololollololol");
          servoRight.write(0 + angleTransparent); // par ex 90 + desired_angle
           servoLeft.write(180 - angleTransparent);
           angle = angleTransparent;
        }
        break;
      case OPAQUE:                     //test if the visor is in opaque state and move it if necessary
          servoRight.write(0 + gradient); // par ex 90 + desired_angle
          servoLeft.write(180 - gradient);
          angle = gradient;
          break;    
    }
    stateFlag = 0;
...
boolean checkVelocity(float _pressure, int _velocity_threshold){
...
//    Serial.print(_pressure);
//    Serial.print(" _velocity_threshold : ");
//    Serial.println(_velocity_threshold);
...
void moveOpen(){
  ...
  Serial.println("Hallllllooooooooooooooooooooooooooooooooooooooooooooooooooooooooo");
}
