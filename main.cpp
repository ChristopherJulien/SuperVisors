...
#define SCL_pin                 A5
#define rain_pin                A6
#define luminosityPot_pin        0  //A0 on the multipexer
#define pressurePot_pin          1  //A1 on the multipexer
...
//----------------------------------------GENERAL VARIABLES--------------------------------------------------//
boolean velocity = false;                        //velocity = 1 if above velocity threshold
boolean rain = false;                            //rain = 1 if rain detected
boolean sun = false;                             //sun = 1 if luminosity above threshold
boolean braking = false;                         //braking = 1 if braking is detected
...
void loop() {
  ...
  if(currentTime- previousTimePotentiometer >= PotentiometerInterval) {
    ...
    Serial.print(velocity_threshold);
  ...
  if(currentTime- previousTimeBraking >= brakingInterval) {
    ...
    doSomethingWithTheBrakingLED(braking);
...
float measurePressure() {
  ...
  return _pressure;  //normalement smoothedPressure
}
...
boolean checkBraking() {
  //determines if the braking value is above the threshold
...
void startContinuousReadingForPressureSensor() {
  ...
  Serial.println(a); //gets a 0 if communication succeeded (3 is also accepted)
}
